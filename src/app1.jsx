...
function Viewport() {
  ...
  useEffect(() => {
    ...
    texARef.current = makeTexture(gl, 0);
    texBRef.current = makeTexture(gl, 1);

    gl.uniform1i(gl.getUniformLocation(programRef.current, 'u_texA'), 0);
    gl.uniform1i(gl.getUniformLocation(programRef.current, 'u_texB'), 1);
  }, []);
  ...
  useEffect(() => {
    ...
    updateTextureFromCanvas(gl, texARef.current, 0, canvasA);
    updateTextureFromCanvas(gl, texBRef.current, 1, canvasB);
  }, [dummy]);
  useEffect(() => {
    const glCanvas = document.getElementById('canvasMain');
    const gl = glCanvas.getContext('webgl');
    console.log(glCanvas.width, glCanvas.height)
    gl.uniform2f(gl.getUniformLocation(programRef.current, 'u_resolution'), glCanvas.width, glCanvas.height);
    gl.uniform1f(gl.getUniformLocation(programRef.current, 'u_tile_size'), tileSize);
    gl.uniform1f(gl.getUniformLocation(programRef.current, 'u_tile_rotation'), tileRotation / 180 * Math.PI);
    gl.uniform2f(gl.getUniformLocation(programRef.current, 'u_tile_offset'), tileOffsetX, tileOffsetY);
...
function App() {
  return <div className='flex'><Viewport /> <Viewport /> <Viewport /></div>
}
